...
undirected-link-breed [coordinators coordinator]
undirected-link-breed [commlinks commlink]
...
patches-own [
  save-convergence-count
  save-experiment-count
]

globals [
  call-for-superpeer
  max-neighbor-degree
  node-neighbor-degree
  experiment-count
  experiment-counter
  convergence-count
  average-convergence
...
;commlinks-own [ weight ]  ;; link breeds can own variables just like turtle breeds

to setup
  set call-for-superpeer 0
  clear-all
  set-default-shape turtles "circle"
  create-turtles initial-node-count [ set color red ]
  ; layout-circle turtles (world-width / 2 - 1)

  ask turtles [
    set size 1
    setxy random-xcor random-ycor
    set superpeerselected ""
    set label superpeerselected
    set search-visit false
  ]

  if (network-architecture = "all-comm-network") [set-comm-network]
  if (network-architecture = "random-network") [set-random-network]

  ask links [
    set distance-bw-nodes 0
  ]

  ; Patch 0 0 is kind enough to store experimental data for us.
  ask patch 0 0 [
    set save-convergence-count []
    set save-experiment-count 0
  ]
...
to go-bully
  ...
  ask turtles [
    if superpeerselected = "COORD" [
      set call-for-superpeer 2
    ]
  ]

  (ifelse
    (call-for-superpeer = 0)
    [
      ; This simulates a node initiating a new coordinator selection
      ; assuming it detects a failed current coordinator
      ask one-of turtles [
        if (is-turtle? turtle (who + 1) and link-with turtle (who + 1) != nobody) [  ; is-turtle? turtle (who + 1) [
          create-coordinator-with turtle (who + 1) [
            set color 46
            set thickness 1.25
          ]
          ask turtle (who + 1) [ set leader? true ]
          set call-for-superpeer 1
        ]
      ]
    ]
    (call-for-superpeer = 1)
    [
      ; find the next largest node ID - and it needs to be via a direct connection/link
      ask turtles with [leader? = true] [
        (ifelse is-turtle? turtle (who + 1)
          [
            (ifelse is-link? link-with turtle (who + 1)
              [
                set leader? false
                create-coordinator-with turtle (who + 1) [
                  set color 46
              ...
              [ ; else if there is still a larger node ID to find, but it's not connected, then just pick another one
                set leader? false
                ask one-of my-links [
                  set random-node-connect other-end ; a global variable as a work around - random-node-connect is a TURTLE
                ]
                create-coordinator-with random-node-connect [  ; one-of both-ends ; other-end one-of both-ends
                  set color 46
                  set thickness 1.25
                  ;set [random-walk-selection?] of random-node-connect true
                ]
                ask random-node-connect [
                  set leader? true
                  ;set random-walk-selection? false
                ]
              ]
             )
          ]
          [ ; else the current turtle is the largest
            ask patch 0 0 [
              set save-convergence-count lput ticks save-convergence-count
              ; show save-convergence-count
              set save-experiment-count (save-experiment-count + 1)
              ; show save-experiment-count
            ]
            set color 65
            set size sqrt count coordinator-neighbors
            set superpeerselected "COORD"
            set label superpeerselected
          ]
        )
      ]
...
; tell everyone in the network you are the new coordinator/leader
to leader-broadcast

end

; the "random walk" nature ensures only a single new link is created per operation
; so we don't have to worry about concurrent connections attempted
to go-pref-attach

 ; this is the generic approach for determining if a new SP is needed,
 ; but it requires pinging on *every cycle* all nodes so it not very efficient
 ; and requires maintaining a global variable, which can't be available in a distributed system
  ask turtles [
    if superpeerselected = "COORD" [
      set call-for-superpeer 2
    ]
  ...
  (ifelse call-for-superpeer = 0 [
    ;; make the initial network of two turtles and an edge
    ; just to get things started: we need one link to enable find-partner
    ; so this is only done one time when a new SP selection is initiated.

    ; This simulates a node initiating a new coordinator selection
    ; assuming it detects a failed current coordinator
    ask one-of turtles [
      create-coordinator-with one-of other turtles [
        set color 46
        set thickness 1.25
      ]
    ]
...
; used for creating a new connection that represents a coordinator "vote"
to make-node [old-node]

  ; show "start make-node" ; testing
  ask one-of turtles ; offers random walk feature
  [
    ; write "make-node with " show self
    if (old-node != self) and (old-node != nobody) [
      create-coordinator-with old-node [
        set color 46
        set thickness 1.25
      ]
    ]
  ...
  ; if old-node has # incoming connections > threshold, then announce it is the superpeer
  ; Alt idea: have it stop when the degree distribution is a certain level -- this would require some global monitor, though
  ; Alt idea: have it stop when all nodes have at least one connection

  ; x --> Alt idea: have it claim SP when its degree is greater than degree of all that it is connected to--> NO: this stops after two connections
  ;set max-neighbor-degree 0
  ;set node-neighbor-degree 0
  ;ask old-node [
  ;  ask my-links [
  ;    ask other-end [
  ;      set node-neighbor-degree count ([link-neighbors]) of self
  ;      if node-neighbor-degree > max-neighbor-degree [
  ;          set max-neighbor-degree node-neighbor-degree
  ;        ]
  ; ] ] ]

  ;; DETERMINE WHEN COORDINATOR IS ELECTED BY THRESHOLD ;;
  ; The Pareto distribution is a power-law / the "80-20 rule" that could be used to approximate
  ; acheiving a scale-free network of the connections -- simply, just get to a number of connections that is 20% of the total possible connections
  if count ([coordinator-neighbors] of old-node) > count turtles * 0.2 [
    ...
    ask old-node [
      ...
      ; report node degrees
      ; write "My SP degree: " show count ([link-neighbors]) of old-node
      ; write "My link-set: " show my-links
      ;ask my-links [ ask other-end [
        ;write "connected node: " write who write " with link degree: " print  count ([link-neighbors] of self)
      ;] ]
    ]
    ; write "Number of steps to SP: " show ticks
  ]
...
to-report find-partner
  ; maybe add in a distance measure here? to that closer is preferred?

  ; this is a random selection with the "weight" toward preference being
  ; if a turtle/node has more links, then it will be more likely to be randomly selected
  report [one-of both-ends] of one-of coordinators

  ; Implementing this is a real network would requires the message making the random walk to
  ; include the "list" of previous nodes connected to so that the next node can make its
  ; random choice from one of these previously selected nodes.
...
;; This layout creates an all-to-all network architecture
to set-comm-network
  ask turtles [
    create-commlinks-with other turtles [
      set color 3
    ]
  ]
end

to set-random-network
  ask turtles [
    set random-connect random initial-node-count ; pick a random number of connections to create
    ; TODO: loop through number of connections to make, randomly select another turtle
    repeat random-connect [
       create-commlink-with one-of other turtles [
          set color 3
       ]
    ]
  ]
...
to resize-nodes
    ...
    ask turtles [
      ifelse count coordinator-neighbors > 1 [
        set size sqrt count coordinator-neighbors
      ]
      [ set size 1 ]
    ]
...
to layout
  ...
  repeat 5 [
    ...
    layout-spring turtles coordinators 0.5 30 1.5;  (1 / factor) (30 / factor) (0.5 / factor)
  ...
  set x-offset limit-magnitude x-offset 0.5
  set y-offset limit-magnitude y-offset 0.5
...
143
Run
if coordinator-election = \"preferential attachment\"\n[ go-pref-attach ]\nif coordinator-election = \"bully algorithm\"\n[ go-bully ]
T
1
T
OBSERVER
...
1496
504
Convergence to Coordinator Selection
Experiment
Messages Set
0.0
10.0
...
"" ""
PENS
"default" 0.5 1 -16777216 true "ask patch 0 0 [ \n set convergence-count last save-convergence-count\n set experiment-count save-experiment-count\n ]" "plotxy experiment-count convergence-count"
"pen-1" 1.0 0 -2674135 true "ask patch 0 0 [\nset average-convergence mean save-convergence-count\n]" "plotxy experiment-count average-convergence"
...
1231
271
Run Coordinator Reset Experiment
\nif call-for-superpeer = 2 [\n\nset call-for-superpeer 0\nask turtles [\n   if superpeerselected = \"COORD\" [ \n       set superpeerselected \"\" \n       set label superpeerselected\n       set color red\n   ]\n   set size 1\n   set leader? false\n]\nclear-links\nreset-ticks\nif network-architecture = \"random-network\" [ set-random-network ]\nif network-architecture = \"all-comm-network\" [ set-comm-network ]\n\n]\n  \nif experiment-count = plan-experiment-count [ stop ]\n  
T
1
T
OBSERVER
...
1492
280
Ave Message Sent Ratio
(average-convergence / count turtles * 100)
2
1
11
...
network-architecture
"all-comm-network" "random-network"
1
...
coordinator-election
"bully algorithm" "preferential attachment"
0
...
232
Min Messages
min [save-convergence-count] of patch 0 0
0
1
11
...
280
Max Messages
max [save-convergence-count] of patch 0 0
0
1
11
